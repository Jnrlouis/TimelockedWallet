...
export default function Home() {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    // If user is not connected to the Goerli Testnet network, let them know and throw an error
    const { chainId } = await web3Provider.getNetwork();
    if (chainId !== 5) {
      window.alert("Change the network to Goerli Test");
      throw new Error("Change network to Goerli Test");
    }
  ...
  const deposit = async () => {
    try {
      ...
      let addMoney = await timelockContract.depositFunds({value: depositAmount});
      setIzLoading(true);
      await addMoney.wait();
      await getBalance();
      await getLocktime();
